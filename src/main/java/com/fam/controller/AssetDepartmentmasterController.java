...
public class AssetDepartmentmasterController {
  ...
  public @ResponseBody String AssetDepartmentMasterDetail(@ModelAttribute AssetDepartment assetDepartment,
  ...
  @RequestMapping(value = "/AssetDepartmentEditGrid.htm", method = RequestMethod.GET)
  public RedirectView AssetDepartmentEditGrid(HttpServletRequest request, HttpServletResponse response,
      RedirectAttributes redirectAttributes) throws Exception {
    RedirectView rv = new RedirectView("GridView.htm");
    redirectAttributes.addFlashAttribute("page", "maker/AssetDepartmentEditGrid");
    ...
    List<AssetDepartment> ls_assetDept = this.assetDepartmentMasterService.getAssetDepartmentlist(recstatus);
    HashMap<String, AssetDepartment> assetDeptMap = new HashMap<String, AssetDepartment>();

    ls_assetDept.forEach(b -> assetDeptMap.put(String.valueOf(b.getRecordid()), b));
    if (ls_assetDept.size() <= 0) {
      redirectAttributes.addFlashAttribute("msg", "*** No Data Available ****");
    }

    Map<String, AssetDepartment> map = new TreeMap<String, AssetDepartment>(assetDeptMap);
    redirectAttributes.addFlashAttribute("list_vc", map);
    redirectAttributes.addFlashAttribute("prev", prev);
    redirectAttributes.addFlashAttribute("Update", Update);
    redirectAttributes.addFlashAttribute("next", next);
